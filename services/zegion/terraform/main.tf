...
resource "aws_spot_instance_request" "zegion_preview" {
  ...
  user_data = <<-EOF
              ...
              cat << 'TELEMETRY_EOF' > /usr/local/bin/velzion-telemetry.sh
#!/bin/bash
while true; do
  CPU=$(top -bn1 | grep "Cpu(s)" | sed "s/.*, *\([0-9.]*\)%* id.*/\1/" | awk '{print 100 - $1}')
  MEM=$(free -m | awk 'NR==2{printf "%.2f", $3*100/$2 }')
  curl -X POST http://54.86.145.100/api/telemetry/ingest \
    -H "Content-Type: application/json" \
    -d "{\"cpu\": $CPU, \"memory\": $MEM}"
  sleep 60
done
TELEMETRY_EOF
              chmod +x /usr/local/bin/velzion-telemetry.sh
              nohup /usr/local/bin/velzion-telemetry.sh > /var/log/velzion-telemetry.log 2>&1 &
              EOF
}
